...
import me.link.bootstrap.infrastructure.mapper.CommunityCommentMapper;
...
public class CommunityCommentServiceImpl extends ServiceImpl<CommunityCommentMapper, CommunityCommentPO> implements CommunityCommentService {
    ...
    public PageResult<CommunityCommentResponseVO> page(CommunityCommentPageRequest request) {
        ...
        Page<CommunityCommentPO> result = page(page, wrapper);
    ...
    public CommunityCommentResponseVO update(Long id, CommunityCommentUpdateRequest request) {
        ...
        comment.setContent(normalizeContent(request.getContent()));
        ApplicationAssert.requireSuccess(updateById(comment), ErrorCode.COMMUNITY_COMMENT_NOT_FOUND);
        return get(id);
    }
    ...
    public void delete(Long id) {
        CommunityCommentPO comment = getRequired(id);
        ensureAuthor(comment, SecurityHelper.getRequiredUserId());
        ApplicationAssert.requireSuccess(removeById(id), ErrorCode.COMMUNITY_COMMENT_NOT_FOUND);
    ...
    private CommunityCommentPO requireParent(Long parentId, CommunityPostPO post, Long tenantId) {
        ...
        CommunityCommentPO parent = getById(parentId);
        if (parent == null

...
import me.link.bootstrap.infrastructure.mapper.CommunityPostMapper;
...
public class CommunityPostServiceImpl extends ServiceImpl<CommunityPostMapper, CommunityPostPO> implements CommunityPostService {
    ...
    public PageResult<CommunityPostResponseVO> page(CommunityPostPageRequest request) {
        ...
        Page<CommunityPostPO> result = page(page, wrapper);
    ...
    public CommunityPostResponseVO update(Long id, CommunityPostUpdateRequest request) {
        ...
        applyMutableFields(post, request.getSectionId(), request.getTopicId(), request.getTitle(), request.getSummary(),
                request.getContent(), request.getCoverUrl());
        ApplicationAssert.requireSuccess(updateById(post), ErrorCode.COMMUNITY_POST_NOT_FOUND);
        return get(id);
    }
    ...
    private void validateTopic(Long topicId, Long sectionId, Long tenantId) {
        ...
        CommunityTopicPO topic = communityTopicService.getById(topicId);
        if (topic == null
